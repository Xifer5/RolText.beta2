...
function useItem(itemId, item) {
  ...
  if (itemId === 'health_potion') {
    ...
    if (item.curesAll || item.curesPoison) {
      if (gameState.playerDebuffs.poison) { delete gameState.playerDebuffs.poison; msgs.push("veneno"); }
    }
    if (item.curesAll || item.curesBurn) {
      if (gameState.playerDebuffs.burn) { delete gameState.playerDebuffs.burn; msgs.push("quemadura"); }
    }
    if (item.curesAll && gameState.playerDebuffs.stun) {
      delete gameState.playerDebuffs.stun; msgs.push("aturdimiento");
    }
    if (item.restoreHp) {
      gameState.player.hp = Math.min(gameState.player.hp + item.restoreHp, gameState.player.maxHp);
    }
    if (msgs.length) addMessage(`Usas ${item.name}. Curas: ${msgs.join(", ")}.`, 'stat');
    else addMessage(`Usas ${item.name}, pero no había efectos activos.`, 'stat');
    if (item.restoreHp) addMessage(`Recuperas ${item.restoreHp} HP.`, 'stat');
...
function equipItem(itemId, item) {
  ...
  if (!slot || !slotTypeMap[slot] || !slotTypeMap[slot].includes(item.type)) {
    addMessage('No puedes equipar ese objeto en este espacio.', 'system');
    return false;
  }
