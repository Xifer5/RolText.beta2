...
- `specs/SPEC-1101-boss-mechanics.md`: bosses con mecánicas únicas (roadmap "siguiente nivel" del usuario, ítem #1) — los 7 bosses de zona/final pasan de stats idénticos + behavior genérico a mecánica propia por boss: Forest Titan (guardia de raíces + Romper Guardia), Cave Devourer (devorar cada 3er turno), Mountain Colossus (coraza 50% física + slam cada 4), Ancient Construct (sobrecarga cada 4), Swamp Abomination (veneno acumulativo), Frost Wyrm (congela magia), Dragon King (3 fases por HP); sistemas nuevos compartidos: Defender universal (3 clases), aturdimiento reusable, congelación de magia, veneno con stacks. ✅ COMPLETA (verificado en vivo, narrativa de Dragon King intacta).
- `specs/SPEC-1102-tactical-decisions.md`: más decisiones tácticas por turno (roadmap ítem #2) — Concentrarse y Golpe Arriesgado (skills universales, panel de habilidades), Interrumpir (botón condicional, cancela la acción cargada del enemigo y resetea su contador), contraataque universal vía Defender (30%, independiente del bono de Duelista), Sangrado (debuff nuevo) + fix del chip de `arcaneFreeze`. ✅ COMPLETA (verificado en vivo, incluido el reset del contador de Cave Devourer al interrumpir).
- `specs/SPEC-1103-enemy-traits.md`: rasgos aleatorios de enemigo regular, no-boss (roadmap ítem #3, rejugabilidad) — 25% de los combates no-boss el enemigo recibe 1 de 5 rasgos (Furioso +35%ATK/-25%DEF, Ladrón roba oro al huir el jugador, Antiguo +30 resist. física/-30 luz vía `getEffectiveResistances()` sin mutar `ENEMY_COMBAT_DATA`, Regenerador regenera salvo quemado recientemente, Cobarde huye bajo 20% HP sin dar recompensa); nombre del enemigo lleva el adjetivo, sin UI nueva. ✅ COMPLETA (verificado en vivo los 5 rasgos + caso sin rasgo; 164/164 tests, incluye 4 nuevos puros).
- `specs/SPEC-1104-narrative-consequences.md`: decisiones narrativas con consecuencias jugables (roadmap ítem #4) — ayudar al viajero → descuento de mercader permanente (10%), robar la bolsa → más emboscadas permanente (+50%), liberar el eco → protección del bosque (-50% solo bioma forest, se compone con lo anterior), absorber el eco → línea de percepción extra de Elara, perdonar mini-boss (botón condicional bajo 25% HP, solo mini-bosses reales vía `isMiniBossId()`, nunca el boss de zona) → reencuentro narrativo que reemplaza su propio slot de spawn, con decisión directa del jugador (aliado/traidor) sin reutilizar el peso de `MORAL_DECISIONS` de endings.js (respeta el "sin karma" de SPEC-0803). ✅ COMPLETA (verificado en vivo las 6 piezas, incluida navegación real de 8 movimientos para forzar el reencuentro; 173/173 tests, incluye 9 nuevos puros).
- `specs/SPEC-1105-buildcraft-specializations.md`: buildcraft con fantasía fuerte por sub-build (roadmap ítem #5) — reemplaza las 9 especializaciones planas de SPEC-0606 por 9 sub-builds con 3 mecánicas interactuantes cada uno: Tanque/Berserker/Caballero Sagrado (Guerrero), Elementalista/Nigromante/Cronomante (Mago), Asesino/Trampero/Duelista (Pícaro); agrega crítico mágico nuevo (para la maldición del Nigromante), sangrado infligible al enemigo (Trampero), turno extra tras lanzar magia (Cronomante); migra saves con especialización legacy a `null`. ✅ COMPLETA (las 9 verificadas en vivo con comparaciones exactas de daño donde aplicó; 176/176 tests).
- `specs/SPEC-1106-behavioral-loot.md`: loot que cambia cómo juegas, no solo números (roadmap ítem #6) — 5 ítems con campo `special` nuevo: Anillo de Último Aliento (sobrevive a 1 HP una vez por combate), Capa de Niebla (evasión extra solo en ronda 1), Espada Voraz (cura al matar, -3 DEF real), Amuleto del Eco (+20% oro/XP si hay algún flag "luz" de `MORAL_DECISIONS`), Libro Quemado (+25% daño de fuego, +30% costo de MP). Drops repartidos por bioma con probabilidad moderada. ✅ COMPLETA (los 5 verificados en vivo con comparaciones exactas; 179/179 tests).
- `specs/SPEC-1107-interactive-travel-events.md`: eventos de viaje más interactivos (roadmap ítem #7) — 4 eventos nuevos con riesgo real dependiente de clase/origen/stats (mismo patrón que "Desprendimiento", el único evento previo con esta mecánica): Luz entre los árboles (Mago/INT), Puente roto (Pícaro/AGI), Altar antiguo (origen aprendiz/INT), Enemigo herido (Guerrero/STR, agrega el par luz/oscuridad `wounded_enemy_spared`/`killed` a `MORAL_DECISIONS`, heredado gratis por el Amuleto del Eco de SPEC-1106). ✅ COMPLETA (4 tests puros nuevos + verificado en vivo el filtro por bioma y la integración con el final; 183/183 tests).
- `specs/SPEC-1108-recurring-rival.md`: rival recurrente (roadmap ítem #8) — Kestrel, personaje 100% nuevo (`js/rivalArc.js`), aparece 3 veces en la cadena principal (tras mq_01 en town, tras mq_03 en garden_1, resolución tras mq_04 en inferno_1). `worldFlags.rival_score` propio (independiente de MORAL_DECISIONS) decide el desenlace: aliado (cura completa + buff de ataque antes del Rey Dragón), traidor (roba 20% del oro), o competidor (duelo real contra el nuevo enemigo `kestrel_rival`). ✅ COMPLETA (verificado en vivo los 3 desenlaces incluido el combate real del duelo; 193/193 tests).
- `specs/SPEC-1109-secondary-objectives.md`: objetivos secundarios por partida (roadmap ítem #9) — 3 "rumores" al azar de las 6 misiones secundarias YA existentes, auto-activados al crear personaje (`rollRumors()` en `quests.js`) en vez de requerir hablarle a un NPC; tracker de misiones subido de 2 a 4 slots. En el camino se encontraron y corrigieron 2 bugs reales preexistentes que los rumores hicieron mucho más probables de disparar: `ui.js` nunca importaba `allItems` (rompía `updateUI()` completo con una misión "collect" activa) y `collect_fairy_dust` apuntaba a un ítem que nunca existió en `allItems`. ✅ COMPLETA (verificado en vivo con ambos bugs corregidos; 199/199 tests).
- `specs/SPEC-1110-emotional-feedback.md`: más feedback emocional en UI (roadmap ítem #10, ÚLTIMO) — texto flotante de daño diferenciado por resultado (crítico/resistido/vulnerable/normal, mismo umbral ±20 que el consejo táctico ya existente), log de combate resalta líneas críticas, animación propia de level-up (1.1s exactos, respetando el borrado forzado a los 1100ms de `showFloatingText()`), toast de "decisión importante" reusando `MORAL_DECISIONS` sin taxonomía nueva, badge persistente de modificadores de partida activos, flash no-bloqueante (~1.4s) al entrar a combate de jefe, y toast ceremonial dorado al vencer un jefe de zona (excluye mini-bosses y al Rey Dragón, que ya tiene epílogo propio). Bug real encontrado y corregido en vivo: `[data-tooltip]{position:relative}` (regla global de tooltips) empataba en especificidad con `.modifier-badge{position:absolute}` y ganaba por orden de declaración, rompiendo el anclaje del badge — corregido subiendo la posición a un selector `#modifier-badge`. ✅ COMPLETA (las 7 piezas verificadas en vivo, incluida la reproducción forzada de los 4 resultados de daño y el toast de jefe capturado vía MutationObserver; 199/199 tests — todo DOM-entangled, sin tests nuevos, mismo precedente aceptado desde SPEC-1101). **Roadmap "siguiente nivel" completo: 10/10.**
